...
def article_collection(request):
    articles_list= ArticlePost.objects.filter(users_like=request.user)
    paginator = Paginator(articles_list, 2)
    page = request.GET.get('page')
    try:
        current_page = paginator.page(page)
        ...
        current_page = paginator.page(paginator.num_pages)
        articles = current_page.object_list
    return render(request, "article/list/article_collection.html", {"articles": articles, "page": current_page})
...
def redit_article(request, article_id):
    if request.method == "GET":
        ...
        try:
            ...
            redit_article.is_check_article='0'
            redit_article.save()
            return HttpResponse("1")
        except:
            return HttpResponse("2")
